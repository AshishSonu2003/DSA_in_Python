...
class Train:
    def __init__(self, train_name, compartment_list=None):
        ...
        if self.__compartment_list is not None:
            for compartment in self.__compartment_list:
                self.add_compartment(compartment)

    def add_compartment(self, compartment):
        if isinstance(compartment, Compartment):
            self.__compartment_list.append(compartment)
        else:
            print("Invalid compartment object")

    def remove_compartment(self, name):
        current_node = self.__compartment_list.head
        prev_node = None

        while current_node is not None:
            if current_node.data.get_name() == name:
                if prev_node is None:
                    self.__compartment_list.head = current_node.next
                else:
        ...
        print("Compartment {} not found".format(name))
        return False

    def insert_compartment_at_beginning(self, compartment):
        if isinstance(compartment, Compartment):
            self.__compartment_list.prepend(compartment)
        else:
            print("Invalid compartment object")
    ...
    def insert_compartment_at_position(self, compartment, position):
        if isinstance(compartment, Compartment):
            self.__compartment_list.insert(position, compartment)
        else:
            print("Invalid compartment object")
    ...
    def check_vacancy(self):
        ...
        return count
train = Train("Express", [])
compartment1 = Compartment("Sleeper", 50, 30)
...
compartment4 = Compartment("Chair Car", 60, 40)
